...
from constants.messages import SUCCESSFUL_UPDATE, REQUIRE_NUMERICAL_VALUE, REMOVAL_MESSAGE, ADMIN_CONFIG, PERSONAL_MESSAGE_AFTER_REMOVAL, INFO_MESSAGE, HELP_MESSAGE, REPORT_MESSAGE, ADMIN_REQUEST_SERVER_ID
...
class ToxicBotAdminCommands(commands.Cog):
    ...
    async def setdays(self, ctx, arg):
        ...
        guild = self.bot.get_guild(int(SERVER_ID))
        guild_name = guild.name if guild is not None else ""
        await ctx.send(SUCCESSFUL_UPDATE.format(entity="Days before resetting toxic count for an user", server=guild_name))
